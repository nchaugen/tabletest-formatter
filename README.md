...
spotless {
    java {
        tableTestFormatter()
        // tableTestFormatter('1.1.2') // pin a specific version
    }
    kotlin {
        tableTestFormatter()
    }
...
```bash
mvn dependency:get \
  -Dartifact=org.tabletest:tabletest-formatter-cli:1.1.2:jar:shaded \
  -Dtransitive=false
```

The JAR will be downloaded to your local Maven repository:
```
~/.m2/repository/org/tabletest/tabletest-formatter-cli/1.1.2/
```
...
```
tabletest-formatter-cli/target/tabletest-formatter-cli-1.1.2.jar
```
